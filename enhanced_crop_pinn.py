...
def group_split_dataset(dataset, train_ratio=0.7, val_ratio=0.15, seed=42):
    """
    Splits data by plot_id so that the same plot does not appear in
...
class CropDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, csv_file):
        ...
        # Sort by plot and time
        self.data = self.data.sort_values(
            ["plot_id", "days_since_planting"]
        ).reset_index(drop=True)

        # ----------------------------------------------------
        # Extra engineered features for accuracy
        # ----------------------------------------------------
        self.data["temp_avg"] = (self.data["tmax"] + self.data["tmin"]) / 2.0

        base_temp = 10.0
        self.data["daily_gdd"] = np.maximum(
            self.data["temp_avg"] - base_temp,
            0
        )

        self.data["growing_degree_days"] = (
            self.data.groupby("plot_id")["daily_gdd"].cumsum()
        )

        self.data["cumulative_rainfall"] = (
            self.data.groupby("plot_id")["rainfall"].cumsum()
        )

        self.data["cumulative_irrigation"] = (
            self.data.groupby("plot_id")["irrigation"].cumsum()
        )

        self.data["cumulative_fertilizer_N"] = (
            self.data.groupby("plot_id")["fertilizer_N"].cumsum()
        )
        ...
        # Fit y scaler only for loss normalization
        self.scaler_y.fit(self.data[self.targets])

        # Keep y in physical units
        self.y = self.data[self.targets].values.astype(np.float32)

        self.weather_raw = self.data[self.weather_features].values.astype(np.float32)
        self.irrigation_raw = self.data["irrigation"].values.astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        if (
            idx + 1 < len(self.data)
            and self.data.iloc[idx + 1]["plot_id"] == plot_id
        ):
            next_idx = idx + 1
            has_next = 1.0
        else:
...
# ============================================================
# 3. ENHANCED CROP PINN MODEL
# ============================================================

class EnhancedCropPINN(nn.Module):

    def __init__(self, input_dim=15, hidden_dims=[256, 128, 64], output_dim=4):
        ...
        self.RUE_raw = nn.Parameter(torch.tensor(0.0))
        self.k_raw = nn.Parameter(torch.tensor(0.0))
        self.Kc_raw = nn.Parameter(torch.tensor(0.0))
        self.SLA_raw = nn.Parameter(torch.tensor(0.0))
        self.T_base_raw = nn.Parameter(torch.tensor(0.0))
        self.T_opt_raw = nn.Parameter(torch.tensor(0.0))
        self.HI_raw = nn.Parameter(torch.tensor(0.0))

    @property
    def RUE(self):
        return 1.5 + (4.5 - 1.5) * torch.sigmoid(self.RUE_raw)

    @property
    def k(self):
        return 0.4 + (0.8 - 0.4) * torch.sigmoid(self.k_raw)

    @property
    def Kc(self):
        return 0.9 + (1.3 - 0.9) * torch.sigmoid(self.Kc_raw)

    @property
    def SLA(self):
        return 0.018 + (0.030 - 0.018) * torch.sigmoid(self.SLA_raw)

    @property
    def T_base(self):
        return 8.0 + (12.0 - 8.0) * torch.sigmoid(self.T_base_raw)

    @property
    def T_opt(self):
        return 23.0 + (28.0 - 23.0) * torch.sigmoid(self.T_opt_raw)

    @property
    def HI(self):
        return 0.35 + (0.60 - 0.35) * torch.sigmoid(self.HI_raw)

    def forward(self, x):

        h1 = self.dropout(self.activation(self.norm1(self.layer1(x))))
        h2 = self.dropout(self.activation(self.norm2(self.layer2(h1))))
        ...
        LAI = 8.0 * torch.sigmoid(output[:, 0])
        biomass = 20000.0 * torch.sigmoid(output[:, 1])
        soil_water = 400.0 * torch.sigmoid(output[:, 2])
        yield_pred = 12000.0 * torch.sigmoid(output[:, 3])

        return torch.stack(
            [LAI, biomass, soil_water, yield_pred],
            dim=1
        )
    ...
    def temperature_stress(self, temp_avg):
        ...
        mask1 = (temp_avg >= self.T_base) & (temp_avg <= self.T_opt)
        stress[mask1] = (
            (temp_avg[mask1] - self.T_base) /
            (self.T_opt - self.T_base + 1e-6)
        )

        mask2 = (temp_avg > self.T_opt) & (temp_avg <= 40.0)
        stress[mask2] = (
            (40.0 - temp_avg[mask2]) /
            (40.0 - self.T_opt + 1e-6)
        )

        stress = torch.clamp(stress, 0.0, 1.0)

        return stress

    def enhanced_physics_loss(
        self,
        predictions,
        predictions_next,
        weather,
        ...
        canopy_mask = (LAI < 7.5) & (time_flat < 60)

        if torch.any(canopy_mask):
            time_normalized = torch.clamp(time_flat[canopy_mask] / 100.0, 0, 1)
            ...
            physics_loss_lai_biomass = torch.mean(
                ((LAI[canopy_mask] - LAI_from_biomass) / 4.0) ** 2
            )
        ...
        if torch.any(mask):
            ...
            physics_loss_water = torch.mean(
                ((water_diff - water_balance_theory[mask]) / 10.0) ** 2
            )

        # ----------------------------------------------------
        # 4. Biomass monotonicity
        # ----------------------------------------------------
        ...
        if torch.any(mask):
            ...
            physics_loss_monotonic = torch.mean(
                (biomass_decrease / 200.0) ** 2
            )
        ...
        if torch.any(grain_filling_mask):
            ...
            grain_fraction = torch.clamp(
                (time_flat[grain_filling_mask] - 60.0) / 40.0,
                0.0,
                1.0
            )
            ...
            yield_decrease = torch.relu(
                yield_pred[grain_filling_mask] -
                yield_next[grain_filling_mask]
            )

            physics_loss_yield_monotonic = torch.mean(
                (yield_decrease / 200.0) ** 2
            )
        ...
        # ----------------------------------------------------
        # 7. Early yield should be close to zero
        # ----------------------------------------------------
        ...
        if torch.any(early_yield_mask):
            physics_loss_early_yield = torch.mean(
                (yield_pred[early_yield_mask] / 300.0) ** 2
            )
        ...
        total_physics_loss = (
            ...
            0.3 * physics_loss_water +
            0.2 * physics_loss_lai_biomass +
            0.1 * physics_loss_monotonic +
            0.05 * physics_loss_senescence +
            0.2 * physics_loss_yield +
            0.1 * physics_loss_early_yield
        )
    ...
    def boundary_loss(self, predictions, time_steps):
        ...
        if torch.any(initial_mask):
            ...
            # Initial LAI near zero
            loss += torch.mean(
                (predictions[initial_mask, 0] / 1.0) ** 2
            )

            # Initial biomass near 50 kg/ha, softer scaling
            loss += torch.mean(
                ((predictions[initial_mask, 1] - 50.0) / 500.0) ** 2
            )

            # Initial yield near zero
            loss += torch.mean(
                (predictions[initial_mask, 3] / 300.0) ** 2
            )
...
def train_pinn_enhanced(
    ...
    for epoch in range(epochs):
        ...
        if epoch < warmup_epochs:
            ...
            lambda_physics = min(
                0.1,
                ((epoch - warmup_epochs) / max(1, epochs - warmup_epochs)) * 0.1
            )
        ...
        # Stronger biomass weight
        weights = torch.tensor(
            [1.0, 1.5, 0.5, yield_weight],
            device=device
        )
        ...
        for batch in train_loader:
            ...
            irrigation = batch["irrigation"].to(device)
            ...
            y_pred = model(X)
            y_pred_next = model(X_next)

            loss_each = huber(y_pred / y_std, y_true / y_std)

            data_loss = torch.mean(
                weights * torch.mean(loss_each, dim=0)
            )

            physics_loss, components = model.enhanced_physics_loss(
                y_pred,
                y_pred_next,
                weather,
                irrigation,
            ...
            for k in components:
                components_epoch[k] += components[k]

        for k in components_epoch:
            components_epoch[k] /= len(train_loader)
        ...
        with torch.no_grad():
            ...
            for batch in val_loader:
                ...
                physics_loss, _ = model.enhanced_physics_loss(
                    y_pred,
                    y_pred_next,
                    weather,
                    irrigation,
        ...
        # Save best model using validation data loss
        if avg_val_data_loss < best_val_data_loss:

            best_val_data_loss = avg_val_data_loss
        ...
        if epoch % 10 == 0 or epoch == epochs - 1:
            print(
                ...
                f"Val Data Loss: {avg_val_data_loss:.4f} | "
                f"Physics λ: {lambda_physics:.4f} | "
                f"Yield W: {yield_weight:.3f}"
            )
...
def predict_with_uncertainty(model, test_loader, device="cpu", n_samples=30):
    ...
    for sample in range(n_samples):
        ...
        prediction_samples.append(np.vstack(preds))

    mean_pred = np.mean(prediction_samples, axis=0)
    std_pred = np.std(prediction_samples, axis=0)
    actuals = np.vstack(actuals)
...
def plot_training_history(history):
    ...
    ax1.plot(history["val_data_loss"], label="Val Data Loss", linewidth=2)
    ...
    ax3.plot(history["learning_rate"], color="green", linewidth=2)
    ...
    component_plots = [
        ("growth", "Growth Physics Loss", "brown"),
        ("water", "Water Balance Physics Loss", "blue"),
        ("lai_biomass", "LAI-Biomass Physics Loss", "green"),
        ("monotonic", "Monotonicity Physics Loss", "purple"),
        ("senescence", "Senescence Physics Loss", "orange"),
        ("yield", "Yield Physics Loss", "goldenrod")
    ]

    for i, (key, title, color) in enumerate(component_plots):
        ...
        ax.plot(
            [comp[key] for comp in history["components"]],
            linewidth=2,
            color=color
        )
    ...
    plt.suptitle(
        "Training History - Enhanced Crop PINN",
        fontsize=16,
        fontweight="bold"
    )
...
def plot_predictions(predictions, actuals):
    ...
    for i, (var, color) in enumerate(zip(variables, colors)):
        ...
        min_v = min(actuals[:, i].min(), predictions[:, i].min())
        max_v = max(actuals[:, i].max(), predictions[:, i].max())
        ...
        axes[i].set_xlabel(f"Actual {var}", fontweight="bold")
        axes[i].set_ylabel(f"Predicted {var}", fontweight="bold")
        axes[i].set_title(f"{var} Predictions", fontweight="bold")
        axes[i].legend()
...
def plot_uncertainty(mean_pred, std_pred, actuals):
    ...
    for i, (var, color) in enumerate(zip(variables, colors)):
        ...
        axes[i].set_title(
            f"{var} Predictions with Uncertainty",
            fontweight="bold"
        )
        axes[i].legend()
        axes[i].grid(True, alpha=0.3)
...
def plot_residuals(predictions, actuals):
    ...
    for i, (var, color) in enumerate(zip(variables, colors)):
        ...
        axes[0, i].set_xlabel(f"Predicted {var}", fontweight="bold")
        axes[0, i].set_ylabel("Residuals", fontweight="bold")
        axes[0, i].set_title(f"{var} Residuals", fontweight="bold")
        ...
        axes[1, i].set_xlabel("Residuals", fontweight="bold")
        axes[1, i].set_ylabel("Frequency", fontweight="bold")
        axes[1, i].set_title(
            f"{var} Residuals Distribution",
            fontweight="bold"
        )
...
def plot_time_series(predictions, actuals, dataset, test_dataset, num_samples=3):
    ...
    selected_ids = np.random.choice(
        unique_ids,
        min(num_samples, len(unique_ids)),
        replace=False
    )
    ...
    for row, plot_id in enumerate(selected_ids):
        ...
        for col, (var, color) in enumerate(zip(variables, colors)):
            ...
            axes[row, col].plot(
                time_subset_sorted,
                actuals[local_indices_sorted, col],
                "o-",
                color="black",
                label="Actual",
                markersize=5,
            ...
            axes[row, col].plot(
                time_subset_sorted,
                predictions[local_indices_sorted, col],
                "s-",
                color=color,
                label="Predicted",
                markersize=5,
...
def plot_learned_parameters(model):
    ...
    for idx, (param_name, ax) in enumerate(zip(params.keys(), axes.flat)):
        ...
        ax.set_xlim(
            exp_min - 0.2 * (exp_max - exp_min),
            exp_max + 0.2 * (exp_max - exp_min)
        )

        ax.set_yticks([])

        if unit:
            ax.set_xlabel(f"{param_name} ({unit})", fontweight="bold")
            ax.set_title(
                f"{param_name} = {value:.3f} {unit}",
                fontweight="bold"
            )
        else:
            ax.set_xlabel(param_name, fontweight="bold")
            ax.set_title(
                f"{param_name} = {value:.3f}",
                fontweight="bold"
            )
...
def generate_synthetic_crop_data(n_samples=5000, output_file="crop_data.csv"):
    ...
    for plot_id in range(n_plots):
        ...
        for day in range(days_per_plot):
            ...
            PAR = solar_radiation * 0.5
            IPAR = PAR * (1.0 - np.exp(-k * LAI))

            daily_growth = RUE * IPAR * temp_stress * 10.0
            ...
            LAI = max(0.0, min(8.0, LAI + LAI_growth))

            biomass = max(
                50.0,
                min(20000.0, biomass + daily_growth)
            )

            # Corrected yield generation:
            # Yield becomes a realistic harvest-index fraction of biomass
            if day > 60:
                grain_fraction = min(1.0, (day - 60.0) / 40.0)
                grain_yield = HI * biomass * grain_fraction
            else:
                grain_yield = 0.0

            grain_yield = min(grain_yield, 12000.0)

            ET0 = (
                0.0023 *
                (temp_avg + 17.8) *
                np.sqrt(max(0.1, tmax - tmin)) *
                solar_radiation *
                0.408
            )

            ET = Kc * ET0 * np.clip(soil_water / 200.0, 0, 1)

            runoff = max(0.0, rainfall - max(0.0, 250.0 - soil_water))

            drainage = 0.05 * max(0.0, soil_water - 250.0)
    ...
    print(df[["LAI", "biomass", "soil_water", "yield"]].describe())
...
def main():
    ...
    # IMPORTANT:
    # Keep True for first run after this correction.
    # Later you can set it to False.
    FORCE_REGENERATE_DATA = True

    if FORCE_REGENERATE_DATA or not os.path.exists("crop_data.csv"):
    ...
    print("Best validation data loss:", checkpoint["val_data_loss"])
    print("=" * 80)
    ...
    print("1. training_history_enhanced.png")
    print("2. predictions_scatter.png")
    print("3. predictions_uncertainty.png")
    print("4. residuals_analysis.png")
    print("5. time_series_predictions.png")
    print("6. learned_parameters.png")
