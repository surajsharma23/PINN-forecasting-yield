...
class CropDataset(torch.utils.data.Dataset):
    def __init__(self, csv_file):
        ...
        # Fit scaler_y to get scale_ and mean_ for loss normalization, but keep y in physical units
        self.scaler_y.fit(self.data[self.targets])
        self.y = self.data[self.targets].values  # Original physical units
        
        # Store raw data for physics calculations
    ...
    def __getitem__(self, idx):
        ...
        # Find next index for the same plot to compute forward differences
...
class EnhancedCropPINN(nn.Module):
    def __init__(self, input_dim=11, hidden_dims=[256, 128, 64], output_dim=3):
    ...
    def enhanced_physics_loss(self, predictions, predictions_next, weather, irrigation, time_steps, has_next):
        ...
        # Convert has_next to boolean mask
        ...
        # Growth potential with temperature stress (g/m²/day)
        ...
        if torch.any(mask):
            ...
            biomass_diff = biomass_next[mask] - biomass[mask]
            
            # Convert units: growth_potential is in g/m²/day, biomass is in kg/ha
            # 1 g/m²/day = 10 kg/ha/day
            growth_potential_scaled = growth_potential[mask] * 10.0
            
            # Normalize by typical growth scale (~100 kg/ha/day) to keep loss stable
            physics_loss_growth = torch.mean(
                ((biomass_diff - growth_potential_scaled) / 100.0)**2
            )
        ...
        # Convert biomass from kg/ha to g/m² for consistency (1 kg/ha = 0.1 g/m²)
        biomass_g_m2 = biomass * 0.1
        
        LAI_from_biomass = biomass_g_m2 * SLA_clamped * partition_coef
        
        # Normalize by typical LAI scale (~4.0)
        physics_loss_lai_biomass = torch.mean(((LAI - LAI_from_biomass) / 4.0)**2)
        ...
        # Water balance theory (including irrigation!)
        ...
        if torch.any(mask):
            ...
            # Normalize by typical daily water change (~5 mm)
            physics_loss_water = torch.mean(
                ((water_diff - water_balance_theory[mask]) / 5.0)**2
            )
        ...
        late_mask = (time_steps.squeeze() > maturity_days) & mask
        ...
        # ==========================================
        # Enforced reliably via Equation 1 and Equation 2
    ...
    def boundary_loss(self, predictions, time_steps):
        ...
        if torch.any(initial_mask):
            ...
            loss += torch.mean(((biomass_initial - 50) / 50.0) ** 2)
...
# ============================================
def train_pinn_enhanced(model, train_loader, val_loader, scaler_y, epochs=100, device='cpu'):
    """
    ...
    # Target normalization scale for data loss
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            X = batch['X'].to(device)
            y_true = batch['y'].to(device)  # Physical units
            weather = batch['weather'].to(device)
            irrigation = batch['irrigation'].to(device)
            time = batch['time'].to(device)
            
            X_next = batch['X_next'].to(device)
            weather_next = batch['weather_next'].to(device)
            irrigation_next = batch['irrigation_next'].to(device)
            ...
            # Data loss: normalize errors so all 3 targets have comparable weight
            weights = torch.tensor([1.0, 0.1, 0.5], device=device)
            normalized_error = (y_pred - y_true) / y_std
            data_loss = torch.mean(weights * torch.mean(normalized_error**2, dim=0))
            
            # Physics loss
            physics_loss, components = model.enhanced_physics_loss(
                y_pred, y_pred_next, weather, irrigation, time, has_next
            )
...
# ============================================
# (Keeping visualization functions identical or updated for physical units)
...
def plot_uncertainty(mean_pred, std_pred, actuals, sample_indices=None):
    ...
    for i, (var, color) in enumerate(zip(variables, colors)):
        ...
        ax.plot(x, actuals[sample_indices, i], 'o', color='black', label='Actual', markersize=4, alpha=0.6)
        ax.plot(x, mean_pred[sample_indices, i], '-', color=color, linewidth=2, label='Predicted Mean')
...
def main():
    ...
    model = EnhancedCropPINN(input_dim=dataset.X.shape[1], hidden_dims=[256, 128, 64], output_dim=3)
    print(f"Total parameters: {sum(p.numel() for p in model.parameters()):,}")
    ...
    history = train_pinn_enhanced(model, train_loader, val_loader, dataset.scaler_y, epochs=200, device=device)
    ...
    predictions, actuals, metrics = evaluate_model(model, test_loader, device=device)
